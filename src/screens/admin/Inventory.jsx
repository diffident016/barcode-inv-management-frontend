...
function Inventory({ user, products, refresh }) {
    ...
    const search = (query) => {
        ...
        temp = temp.filter((product) => {
            ...
            var category = product.category.category_name.toLowerCase().indexOf(query.toLowerCase());
    ...
    const columns = useMemo(
        () => [
            ...
            {
                name: "Stock",
                cell: (row) => <p className='w-full text-sm overflow-hidden text-ellipsis'>{row.stock - row.sold}/{row.stock}</p>,
                width: '100px'
            },
            {
                name: "Barcode",
        ...
        if (state == 2) {
            return (
                <div className='w-full h-full flex flex-col items-center pt-4'>
                    ...
                    <h1 className='font-lato text-lg '>Your have no products, add now!</h1>
                </div>
            )
        }
            ...
            <div className='flex flex-col flex-1 h-full rounded-lg border bg-white py-4 px-6'>
                ...
                <div className='flex flex-row justify-between w-full items-center pt-4 pb-2'>
                    ...
                    <div className='flex flex-row gap-2 select-none items-center px-2'>
                        ...
                        <h1
                            onClick={() => { setAddProduct({ add: true, barcode: null }) }}
                ...
                <div className='flex flex-col w-full h-full px-2 overflow-hidden'>
                    ...
                    {products.fetchState != 1 ?
                        ...
                        : <div className='flex flex-col overflow-auto h-full w-full'>
                            {
                                (!!newProducts ? newProducts : products['products']).map((item, index) => {
                                    return (
                                        <div
                                            onClick={() => {
                                                setShowProduct(item);
                    ...
                    <Backdrop
                        ...
                        open={!!showProduct}
                    >
                        {!!showProduct && <ProductDetails refresh={() => { refresh(); }} product={showProduct} close={() => { setShowProduct(null) }} />}
                    </Backdrop>
                </div>
            </div>
        </div>
