...
function Purchase({ orders, refresh }) {
        ...
        if (state == 2) {
            return (
                <div className='w-full h-full flex flex-col items-center pt-4'>
                    ...
                    <h1 className='font-lato text-lg '>You have no orders yet, just wait and chill.</h1>
                </div>
            )
        }
    ...
    const orderStatus = (state, date) => {
            ...
            return (
                <div className='flex flex-col w-full font-lato gap-1'>
                    <div className='flex flex-row w-full items-center gap-2'>
                        ...
                        <h1 className='text-sm py-[2px] font-lato-bold text-[#49a54d]'>Order process completed</h1>
                    </div>
                    <p className='text-sm font-lato'>Completed on: <span className='font-lato-bold'>{format(date, 'dd MMM yyyy - p')}</span></p>
                </div>)
            ...
            return (
                <div className='flex flex-col w-full font-lato gap-1'>
                    <div className='flex flex-row w-full items-center gap-2'>
                        ...
                        <h1 className='text-sm py-[2px] font-lato-bold text-[#fb0200]'>Order canceled by customer</h1>
                    </div>
                    <p className='text-sm'>Canceled on: <span className='font-lato-bold'>{format(date, 'dd MMM yyyy - p')}</span></p>
                </div>)
        }
        ...
        if (state == 4) {
            return (
                <div className='flex flex-col w-full font-lato gap-1'>
                    <div className='flex flex-row w-full items-center gap-2'>
                        <p className='text-sm text-center'>Status: </p>
                        <h1 className='text-sm py-[2px] font-lato-bold text-[#fb0200]'>Order canceled by store</h1>
                    </div>
                    <p className='text-sm'>Canceled on: <span className='font-lato-bold'>{format(date, 'dd MMM yyyy - p')}</span></p>
                </div>)
        }
        ...
        return (
            <div className='flex flex-col w-full font-lato gap-1'>
                <div className='flex flex-row w-full items-center gap-2'>
                    <p className='text-sm text-center'>Status: </p>
                    <h1 className='text-sm py-[2px] font-lato-bold text-[#ffc100]'>Pending order approval</h1>
                </div>
            </div>)
    }
    ...
    const handleAction = (type, order) => {
        ...
        if (type == 0) {
            cancelOrder(order._id, 4).then((res) => res.json()).then((val) => {
            ...
            completeOrder(order).then((res) => res.json()).then((val) => {
    ...
    const search = (query) => {
        ...
        temp = temp.filter((order) => {
            ...
            var customer = order.customer.name.toLowerCase().indexOf(query.toLowerCase());
            var category = order.product.category.category_name.toLowerCase().indexOf(query.toLowerCase());
    ...
    const columns = useMemo(
        () => [
                ...
                cell: (row, index) => {
                    return (
                        <div className='flex flex-row h-full w-full items-center justify-center gap-5'>
                            <img className='w-20 border rounded-lg shadow-sm' src={row.product['photoUrl']} alt='product' />
                            <div className='w-full flex flex-col gap-2 py-2 text-sm'>
                                <h1 className='font-lato-bold '>{row.product.name}</h1>
                                <h1 className='font-lato-bold'>{row.product.price.toLocaleString('en-US', {
                                    style: 'currency',
                                    currency: 'PHP',
                                })}</h1>
                            </div>
            ...
            {
                cell: (row) => <p className='w-full font-lato text-sm text-center'>Qty: {row.quantity}</p>,
                width: '110px'
            },
            {
                cell: (row) => <p className='w-full font-lato-bold text-center'>{row.orderAmount.toLocaleString('en-US', {
                    style: 'currency',
                    currency: 'PHP',
                })}</p>,
                width: '200px'
                ...
                cell: (row) => {
                    ...
                    return (
                        <div className='flex flex-col gap-3'>
                            ...
                            {(row.orderStatus < 3 && row.orderStatus !== 2) && <div className='flex flex-row text-sm font-lato-bold gap-2 '>
                                <button
                                    onClick={() => {
                                        setShowDialog({
                                            title: 'Cancel Order',
        ...
        <div className='w-full h-full p-4'>
            <div className='w-full h-full flex flex-col text-[#555C68] gap-2     ' >
                <div className='w-full bg-white border rounded-lg px-4 py-4'>
                    <h1 className='font-lato-bold text-base'>Customer Purchase</h1>
                    <div className='flex flex-row justify-between w-full items-center pt-3'>
                        ...
                        <div className='flex flex-row gap-2 select-none items-center px-2'>
                            <h1
                                ...
                                <span>{<ArrowPathIcon className='w-5' />}</span> Refresh</h1>
                        </div>
                    </div>
                </div>
                {
                    orders['fetchState'] != 1 ? statusBuilder(orders['fetchState']) :
                                                ...
                                                <div className='relative'>
                                                    {!!selected && (selected._id == item._id) &&
                                                        <div className='absolute w-full h-full rounded-lg bg-white/50 flex justify-center items-center'>
                                                            <CircularProgress size={28} color='inherit' className='text-[#ffc100]' />
                                                        </div>
                                                    }
                                                    <div
                                                        key={item._id}
                                                        ...
                                                        <div className='flex flex-row justify-between items-center'>
                                                            <div className='flex flex-row items-center gap-1'>
                                                                ...
                                                                <p className='text-sm font-lato-bold'>{item.customer.name}</p>
                                                                <p className='text-sm font-lato'>/ {format(item.orderDate, 'dd MMM yyyy - p')}</p>
                                                            </div>
                                                            <p className='text-sm font-lato'>Order ID: <span className='underline cursor-pointer'>{item._id}</span></p>
                                                        </div>
                                                        ...
                                                        <div className='w-full flex flex-row p-2'>
                                                            {
                                                                columns.map((col, index) => {
                                                                    return (
                                                                        <div key={item._id + index} style={{ width: col.width }} className='flex items-center'>
                                                                            {col.cell(item, index)}
                                                                        </div>
                                                                    )
                                                                })
                                            ...
                                            orderFilter.map((item) => {
                                                return (
                                                    <div
                                                        ...
                                                        key={item.status}
                                                        className={` ${filter == item.status ? ' border-[#ffc100] text-[#ffc100] bg-[#ffc100]/10' : 'border-white'}  border-l-[3px] h-11 w-full px-4 flex items-center cursor-pointer hover:bg-[#ffc100]/10`}>
